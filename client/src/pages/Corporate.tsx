...
export default function Corporate() {
  ...
  return (
    <div className="min-h-screen bg-[var(--brac-surface-10)]">
      ...
      <div className="border-b border-[var(--brac-border)]">
              ...
              <button
                ...
                className={`px-6 py-4 font-['Barlow_Condensed'] font-bold text-sm tracking-wide uppercase border-b-2 transition-all whitespace-nowrap ${
                  activeTab === tab.id
                    ...
                    : "border-transparent text-[var(--brac-text-muted)] hover:text-[var(--brac-text-dim)]"
                }`}
              >
                {asString(tab.label)}
              </button>
      ...
      <div className="container mx-auto px-6 max-w-7xl py-20" ref={ref as React.RefObject<HTMLDivElement>}>
        {activeTab === "about" && (
          <div className={`transition-all duration-500 ${isVisible ? "opacity-100" : "opacity-0"}`}>
            <div className="max-w-3xl">
              <h2 className="font-['Barlow_Condensed'] font-black text-[var(--brac-on-surface)] text-4xl mb-6 uppercase">
                {asString(tabs.find((tab) => tab.id === "about")?.title, "Hakkımızda")}
              </h2>
              <div className="space-y-4 text-[var(--brac-text-muted)] font-['Inter'] leading-relaxed">
            ...
            <div className="relative pl-6 border-l border-[var(--brac-border-strong)]">
              {milestones.map((milestone, index) => (
                <div key={`${milestone.year}-${index}`} className="relative mb-10 last:mb-0">
                  <div className="absolute -left-[1.625rem] w-3 h-3 bg-[oklch(0.60_0.18_42)] border-2 border-[var(--brac-surface-10)]"></div>
                  <div>
                    <div className="font-['Barlow_Condensed'] font-black text-[oklch(0.60_0.18_42)] text-xl mb-2">
                      {asString(milestone.year)}
                    </div>
                    <p className="text-[var(--brac-text-muted)] font-['Inter']">{asString(milestone.text)}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {boardMembers.map((member, index) => (
                <div
                  ...
                  className="bg-[var(--brac-surface-14)] border border-[var(--brac-border)] p-6 hover:border-[oklch(0.60_0.18_42)] transition-colors"
                >
                  ...
                  <h3 className="font-['Barlow_Condensed'] font-bold text-[var(--brac-on-surface)] text-lg mb-1 uppercase">
          ...
          <div className={`transition-all duration-500 ${isVisible ? "opacity-100" : "opacity-0"}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {certifications.map((cert, index) => (
                <div
                  key={`${cert.name}-${index}`}
                  className="bg-[var(--brac-surface-14)] border border-[var(--brac-border)] p-8 hover:border-[oklch(0.60_0.18_42)] transition-colors"
                >
